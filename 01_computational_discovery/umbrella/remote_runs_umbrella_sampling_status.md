...
Current umbrella progress: old/default umbrella compute remains guarded after repeated WHAM overlap/bin warnings. Paired `LiDA-1`, `LiDS-1`, and `LiD3-Flex` V2 windows are active in both conditions. LiCl `LiDA-1` remains `9/27` and is running windows `009-011`; LiCl `LiDS-1` remains `8/27`; LiCl `LiD3-Flex` is running windows `000-003`; NaCl `LiDA-1` advanced to `16/22` and is running windows `016-019`; NaCl `LiDS-1` remains `12/27`; NaCl `LiD3-Flex` is running windows `000-001`. Old windows and repair outputs are retained only as diagnostics/preliminary QC evidence.
