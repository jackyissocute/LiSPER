...
Scientific steward snapshot: 2026-07-17 08:58 CST
...
| Stage | 16/16 pulls complete; 480/480 windows generated; 376 equilibrations + 144 production complete; 23 equilibration + 103 production active |
...
| Measured horizon | 520 endpoint-valid stages average ≈73,667 atom·ns/day (≈9.28M atom·ns/day at 126 jobs); 1,000 current-queue campaign-block resamples give median table ETA ≈2026-07-20 23:21 CST (90% band 23:00–23:39 CST), recalibrated as production evidence grows |
| Next | Keep the verified 126-job pool full and run paired WHAM only after both ion campaigns complete |

All 126 jobs consumed CPU during a five-second `/proc` check and remain unique. Scheduler caps were re-ranked without interrupting any `mdrun`: LiA3-Ref/NaCl and LiN3-Core/NaCl use the 125-job ceiling, while LiD3-Core/NaCl now owns the 126th slot because it has the greatest remaining atom-weighted work after LiA3-Ref/NaCl. The shared driver prevents a restarted scheduler from duplicating an already-active window. Completion requires both the configured final step and the GROMACS finish marker; no active log tail contains a fatal, water-SETTLE failure, or LINCS warning.
...
| LiND-Hybrid | LiCl | 30/30 | 0 | 0 | 12/30 | 18 | 18 |
| LiND-Hybrid | NaCl | 30/30 | 0 | 0 | 15/30 | 15 | 15 |
