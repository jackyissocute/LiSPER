# Umbrella Sampling Status

Scientific steward snapshot: 2026-07-24 07:05 CST

## Live state

| Item | Value |
|---|---|
| Active host | EPYC 9554P, 128 hardware threads |
| Campaigns | 8 candidates × LiCl/NaCl = 16 independent paired-site campaigns |
| Stage | 16/16 pulls complete; 480/480 windows generated; 479 equilibrations complete, 1 active; 384 production complete, 95 active |
| Real GROMACS work | 96 unique one-thread `mdrun`: 1 EQ window + 95 production windows; 96/126 umbrella-MD threads in 96 distinct directories, with all currently ready windows active |
| Reserved support | 0/2 threads active; all campaign drivers are resumed and the off-host restart sync is idle between successful passes |
| Bound starts | 16/16 regenerated, minimized, and validated without `-maxwarn` |
| Window protocol | 0.075 nm spacing; 0.5 ns equilibration; 2.0 ns production; 3 endpoint guards |
| Pre-WHAM gate | GROMACS 2026.0 `-ac` with retained `-oiact`/ACF evidence; production bootstrap is explicitly `-bs-method traj` |
| Measured horizon | Progress-adjusted queue = 3.93M atom·ns. The latest 183-minute same-host/protocol progress basis is ≈9.10M atom·ns/day aggregate, giving median table ETA 2026-07-24 17:26 CST; ±20% throughput sensitivity gives 2026-07-24 15:43 to 2026-07-24 20:02 CST |
| Next | Keep every newly unblocked window running; the pool is now dependency-limited below 126, so production throughput will recalibrate |

A five-second `/proc` sample found all 96 real GROMACS executables advancing, with 96 actual MD threads and no duplicate working directory. All currently ready windows are active; one production window remains blocked behind the final active EQ stage. All 16 pulls independently verified configured step 500,000 plus `Finished mdrun`; each generated 30 windows. Four hundred seventy-nine EQ windows and three hundred eighty-four production windows verified their configured final step plus `Finished mdrun`; 95 production windows are active. No active log contains a fatal, water-SETTLE, or LINCS warning. LiDA-1/NaCl remains at 30/30 production, but paired WHAM remains gated on LiDA-1/LiCl at 29/30; its final window is advancing. The off-host restart sync completed multiple successful passes after its last transient timeout and is idle between passes. Production throughput will recalibrate the forecast.

## Candidate window counts

| Candidate | Solution | EQ complete/30 | EQ left | EQ active | Production complete/30 | Production left | Production active |
|---|---|---:|---:|---:|---:|---:|---:|
| LiA3-Ref | LiCl | 30/30 | 0 | 0 | 22/30 | 8 | 8 |
| LiA3-Ref | NaCl | 30/30 | 0 | 0 | 10/30 | 20 | 20 |
| LiD3-Core | LiCl | 30/30 | 0 | 0 | 26/30 | 4 | 4 |
| LiD3-Core | NaCl | 30/30 | 0 | 0 | 15/30 | 15 | 15 |
| LiD3-Flex | LiCl | 30/30 | 0 | 0 | 28/30 | 2 | 2 |
| LiD3-Flex | NaCl | 30/30 | 0 | 0 | 29/30 | 1 | 1 |
| LiDA-1 | LiCl | 30/30 | 0 | 0 | 29/30 | 1 | 1 |
| LiDA-1 | NaCl | 30/30 | 0 | 0 | 30/30 | 0 | 0 |
| LiDS-1 | LiCl | 30/30 | 0 | 0 | 26/30 | 4 | 4 |
| LiDS-1 | NaCl | 30/30 | 0 | 0 | 26/30 | 4 | 4 |
| LiLC-1 | LiCl | 30/30 | 0 | 0 | 14/30 | 16 | 16 |
| LiLC-1 | NaCl | 30/30 | 0 | 0 | 23/30 | 7 | 7 |
| LiN3-Core | LiCl | 29/30 | 1 | 1 | 23/30 | 7 | 6 |
| LiN3-Core | NaCl | 30/30 | 0 | 0 | 27/30 | 3 | 3 |
| LiND-Hybrid | LiCl | 30/30 | 0 | 0 | 26/30 | 4 | 4 |
| LiND-Hybrid | NaCl | 30/30 | 0 | 0 | 30/30 | 0 | 0 |

## Analysis contract

`evaluate_paired_pmf_qc.py` outputs the radially corrected, endpoint-referenced PMF binding differences and paired Delta Delta G whenever profiles exist. Negative Delta Delta G means Li preference. WHAM uses per-window IACT weighting with retained IACT/ACF evidence and trajectory bootstrap; histogram support, endpoint span, early/late difference, burn-in sensitivity, and bootstrap uncertainty remain numerical diagnostics rather than invented universal PASS gates.

These PMF values support within-protocol Li/Na selectivity comparisons. They are not labeled as 1 M standard binding free energies.
