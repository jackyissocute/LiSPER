...
Last updated: 2026-07-10 18:48 CST
...
| `LiD3-Flex` | LiCl | GCP | V2 `21/27` | production windows `016-019` active | `🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜` |
| `LiD3-Core` | LiCl | GCP | V2 `12/27` | production windows `012-013` active | `🟩🟩🟩🟩🟩🟩🟩🟦⬜⬜` |
| `LiLC-1` | LiCl | GCP | V2 `4/27` | production window `004` active | `🟩🟩🟩🟩🟦⬜⬜⬜⬜⬜` |
| `LiN3-Core` | LiCl | GCP | V2 `4/27` | production window `004` active | `🟩🟩🟩🟩🟦⬜⬜⬜⬜⬜` |
| `LiA3-Ref` | LiCl | GCP | V2 `4/27` | equilibration window `004` active | `🟩🟩🟩🟩🟦⬜⬜⬜⬜⬜` |
...
| `LiD3-Flex` | NaCl | GCP | V2 `22/27` | production windows `022-026` active | `🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜` |
| `LiLC-1` | NaCl | GCP | V2 `12/27` | production windows `012-013` active | `🟩🟩🟩🟩🟩🟩🟩🟦⬜⬜` |
| `LiA3-Ref` | NaCl | GCP | V2 `12/27` | production windows `012-013` active | `🟩🟩🟩🟩🟩🟩🟩🟦⬜⬜` |
| `LiD3-Core` | NaCl | GCP | V2 `12/27` | production windows `012-013` active | `🟩🟩🟩🟩🟩🟩🟩🟦⬜⬜` |
| `LiN3-Core` | NaCl | GCP | V2 `12/27` | equilibration windows `012-013` active | `🟩🟩🟩🟩🟩🟩🟩🟦⬜⬜` |
| `LiND-Hybrid` | NaCl | GCP | V2 `4/27` | production windows `004-005` active | `🟩🟩🟩🟩🟦⬜⬜⬜⬜⬜` |

Current umbrella progress: active compute is on the 32-core GCP runner. Paired refined windows are active without duplicate candidate-condition-stage jobs. Complete-window counters here are audited from real completed `umbrella.gro`/done outputs, while active windows are listed separately. LiCl `LiDA-1` completed V4 window `026` repair and combined WHAM/bootstrap under `pmf_wham_v4_20260702_1855`; the V4 profile has `200/200` finite points but still has `12` poor-sampling warning lines at `z=2.24551-2.25665 nm` and a `2.73 kJ/mol` burn-in/time-slice span shift, so classification remains `REPAIR` pending materiality/next-repair review. `LiDS-1` has paired V2 `27/27` windows complete; LiCl WHAM/bootstrap completed under `pmf_wham_v2_20260703_0655` with `200/200` finite points, `22` warning lines, and poor/empty far-tail bins near `z=2.48-2.53 nm`, so classification is QC review and no Delta G is promoted. This heartbeat advanced `LiN3-Core` NaCl to V2 `12/27` and moved `LiND-Hybrid` LiCl/NaCl active windows into production; WHAM/QC artifacts did not materially change. Old windows and repair outputs are retained only as diagnostics/preliminary QC evidence.

Capacity/progress check, 2026-07-10 18:48 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `92%` used. No extra job was submitted because active drivers are already occupying ready work and disk pressure is high.

Capacity/progress check, 2026-07-10 14:43 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `90%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-10 05:41 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `88%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-10 01:38 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `87%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-09 21:33 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `86%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-09 17:29 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `84%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-09 13:24 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `83%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-09 09:19 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `82%` used. No extra job was submitted because active drivers are already occupying ready work.

Capacity/progress check, 2026-07-09 05:14 CST: GCP is healthy at `25` real workflow `mdrun` jobs using `25` OpenMP threads, load near `25`, and data disk at `81%` used. No extra job was submitted because active drivers are already occupying ready work.
