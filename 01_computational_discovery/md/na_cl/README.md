...
| 20 ns production | 6/8 active across both workers: 4 Worker B jobs at `9.45-14.92 ns / 20 ns`, plus 2 Worker A backfill jobs at `3.14-4.74 ns / 20 ns` as of `2026-06-22 15:26 CST`; `LiDA-1` and `LiDS-1` complete |
...
|---|---|
| `LiD3-Core` | Active checkpoint resume on Worker B; `14.16 ns / 20 ns`; clustering queued |
| `LiD3-Flex` | Active checkpoint resume on Worker B; `9.45 ns / 20 ns`; clustering queued |
| `LiND-Hybrid` | Worker A backfill active; `3.14 ns / 20 ns`; clustering queued |
| `LiLC-1` | Active checkpoint resume on Worker B; `14.92 ns / 20 ns`; clustering queued |
...
| `LiN3-Core` | Worker A backfill active; `4.74 ns / 20 ns`; clustering queued |
| `LiA3-Ref` | Active checkpoint resume on Worker B; `13.78 ns / 20 ns`; clustering queued |
