...
| 20 ns production | Worker B active at `12/12` cores with 4 checkpoint-resumed jobs at `10.77-16.86 ns / 20 ns`; replacement Worker A backfill active at `3.72-5.64 ns / 20 ns` as of `2026-06-23 03:24 CST`; `LiDA-1` and `LiDS-1` complete |
...
|---|---|
| `LiD3-Core` | Recovered checkpoint resume on Worker B; `16.07 ns / 20 ns`; clustering queued |
| `LiD3-Flex` | Recovered checkpoint resume on Worker B; `10.77 ns / 20 ns`; clustering queued |
| `LiND-Hybrid` | Replacement Worker A backfill active; `3.72 ns / 20 ns`; clustering queued |
| `LiLC-1` | Recovered checkpoint resume on Worker B; `16.86 ns / 20 ns`; clustering queued |
...
| `LiN3-Core` | Replacement Worker A backfill active; `5.64 ns / 20 ns`; clustering queued |
| `LiA3-Ref` | Recovered checkpoint resume on Worker B; `15.58 ns / 20 ns`; clustering queued |
