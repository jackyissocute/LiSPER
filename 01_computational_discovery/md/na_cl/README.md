...
| 20 ns production | Worker B active at `12/12` safe mdrun threads with 4 checkpoint-resumed jobs at `12.62-19.67 ns / 20 ns`; replacement Worker A backfill active at `4.52-6.88 ns / 20 ns` as of `2026-06-23 20:41 CST`; `LiDA-1` and `LiDS-1` complete |
| Structural clustering | 2/8 complete; `LiDA-1` top cluster `17.94%`; `LiDS-1` top cluster `14.59%`; others queued after production |
| PMF handoff | Umbrella windows: `LiDA-1` `15/15` complete with preliminary WHAM QC warning; `LiDS-1` `9/17`, active `009-012` |
