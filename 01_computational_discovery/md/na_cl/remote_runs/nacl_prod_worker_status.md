...
| Launch state | Worker B active: 1 production job + active `LiLC-1`, `LiA3-Ref`, and repaired `LiD3-Core` NaCl umbrella windows; `LiDA-1` NaCl repair extensions completed; replacement Worker A backfill active for two NaCl jobs |
...
| `LiDA-1` | `20.00 ns / 20 ns`; representative ready; top cluster `17.94%`; umbrella `15/15` complete; WHAM QC repair extensions `5/5` complete; combined WHAM/bootstrap QC queued |
