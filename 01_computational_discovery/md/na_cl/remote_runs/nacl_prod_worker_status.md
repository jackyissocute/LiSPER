...
| Production progress | Worker B jobs `12.31-19.20 ns / 20 ns`; Worker A backfill jobs `4.38-6.66 ns / 20 ns`; `LiDA-1` and `LiDS-1` complete |
...
- PBC-safe umbrella windows are complete for `LiDA-1` NaCl (`15/15`) with WHAM QC warnings. `LiDS-1` NaCl has `6/17` windows complete and active windows `006-009`.
