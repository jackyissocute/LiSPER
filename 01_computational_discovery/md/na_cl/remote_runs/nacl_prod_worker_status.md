...
| Launch state | Worker B active: 1 production job + 2 active `LiLC-1` windows + `LiA3-Ref` and repaired `LiD3-Core` NaCl pulls; replacement Worker A backfill active for two NaCl jobs |
...
| Worker pool | Worker B: 1 active production job x 2 OpenMP threads, 2 active `LiLC-1` NaCl windows x 1 thread, and 2 active NaCl pulls x 1 thread |
| Effective CPU quota | Worker B is at 6/12 active mdrun threads during the two pull stages and should ramp as those pulls convert into window batches; replacement Worker A confirmed at 18/18 cores |
