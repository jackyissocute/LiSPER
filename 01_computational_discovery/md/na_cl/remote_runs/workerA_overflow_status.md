...
| Candidate | State |
|---|---|
| `LiND-Hybrid` | Active on replacement Worker A backfill; `4.38 ns / 20 ns`; `-ntomp 1` |
| `LiN3-Core` | Active on replacement Worker A backfill; `6.66 ns / 20 ns`; `-ntomp 1` |

## Scheduling Note

Replacement Worker A now runs two LiCl production jobs at 2 threads each plus two NaCl backfill jobs at 1 thread each plus twelve LiCl umbrella windows at 1 thread each, using 18/18 cores without duplicating Worker B. Worker B continues four NaCl production jobs at 2 threads each plus four `LiDS-1` NaCl umbrella windows at 1 thread each, using 12/12 cores.
