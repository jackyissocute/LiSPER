...
- Completed 20 ns LiCl production and peptide-only clustering for LiND-1.

Running:
- `IDP-Li-2` production MD is active through the corrected topology-path workflow.
...
- Peptide-only clustering completed for `LiD3-1`, `LiND-1`, `IDP-Li-1`, and `StrongBind-Li`; all four now have representative structures.
- `IDP-Li-2` previously failed production setup at `gmx grompp` because `toppar/forcefield.itp` was not found, but it is now running through the corrected topology-path workflow.
- `LowCharge-Li` and `LiD2-IDP` also failed production setup at `gmx grompp` because `toppar/forcefield.itp` was not found and are expected to use the corrected workflow later.
- `LiD3-1`, `LiND-1`, `IDP-Li-1`, and `StrongBind-Li` top-cluster populations are low, so umbrella sampling should use these representatives carefully and may benefit from additional-cluster comparison.
...
- Completed LiND-1 production small outputs synced under `remote_results/systems/LiND-1/gromacs/run_prod_20ns/`.
- LiND-1 representative structure synced at `remote_results/systems/LiND-1/gromacs/cluster_20ns/representative_top_cluster.pdb`.
...
- After `IDP-Li-2` finishes, run peptide-only clustering and continue the corrected LiCl production queue.
...
- Do not interrupt the active `IDP-Li-2` production run.
- Keep the old parent driver frozen; do not resume it.
- Let the corrected production/clustering workflow continue after `IDP-Li-2` completes.
...
- For umbrella sampling, inspect whether the low top-cluster populations for `LiD3-1`, `LiND-1`, `IDP-Li-1`, and `StrongBind-Li` justify including additional cluster representatives rather than only the largest cluster.
