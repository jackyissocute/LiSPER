...
Latest production snapshot: `2026-06-19 11:50 CST`.

| Condition | Folder | Current state |
|---|---|---|
| LiCl | `li_cl/` | 8/8 production jobs active; `2.28-7.86 ns / 20 ns`; clustering queued after each production run |
| NaCl | `na_cl/` | 6 production jobs active + 2 queued after checkpoint-safe optimization; active jobs `0.44-1.74 ns / 20 ns`; queued jobs `0.13-0.16 ns / 20 ns`; clustering queued after each production run |
