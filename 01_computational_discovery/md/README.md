...
Latest production/umbrella snapshot: `2026-06-24 09:28 CST`.
...
| NaCl | `na_cl/` | Worker B has one production job at `14.19 ns / 20 ns`, two NaCl umbrella pulls, and two `LiLC-1` windows; Worker A backfill is `5.09-7.78 ns / 20 ns`; 5/8 representatives ready |
| Umbrella | `remote_runs_umbrella_sampling_status.md` | `56` current windows complete; active LiCl windows, active `LiLC-1` NaCl windows, and new `LiA3-Ref`/`LiD3-Core` NaCl pulls; NaCl `LiDA-1` preliminary WHAM QC has warnings |
