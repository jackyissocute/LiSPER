# Track B: Surface Display Validation

Track B answers the biological-deployment question:

> Can LiSPER remain functional when displayed on a biological surface?

This track uses future eCPX surface-display constructs to test surface expression, surface accessibility, whole-cell Li+ capture, and Li+/Na+ selectivity.

Track B is not a fallback for Track A. It answers a different question.

## Contents

| Folder | Purpose |
|---|---|
| `plasmids/` | Placeholder for future eCPX construct designs. No eCPX plasmids are designed yet. |
| `protocols/` | Track B protocol overview and future display workflow protocols. |
| `assays/surface_display_assays/` | Whole-cell Li+/Na+ binding and selectivity assay design package. |
| `research/surface_display_host_selection/` | Host, environment, and display-platform technology assessment. |

## Start Here

1. Read `protocols/00_track_B_surface_display_protocol_overview.md`.
2. Read `assays/surface_display_assays/whole_cell_LiNa_binding_assay_plan.md`.
3. Use `assays/surface_display_assays/construct_design_implications.md` before any future eCPX plasmid design.

