...
**Last synchronized monitor snapshot:** `2026-06-21 02:41 CST`
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>7.33-17.29 ns / 20 ns</code> across 7 active jobs; <code>LiDA-1</code> produced</td>
    ...
    <tr>
      ...
      <td><code>🟩⬜⬜⬜⬜⬜⬜⬜</code> 1/8 representative complete; top cluster <code>17.64%</code></td>
      <td><img alt="clustered" src="https://img.shields.io/badge/clustered-1%2F8-16a34a"></td>
      <td>Li+ representative structures; <code>LiDA-1</code> ready for paired NaCl gate</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>20 ns production MD</td>
      <td><code>4.83-17.15 ns / 20 ns</code> across 6 Worker B jobs; 2 backfill jobs active at <code>0.78-1.08 ns</code></td>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>🟦 <code>11.15 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>7.25 ns / 20 ns</code>; clustering queued</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
    <tr>
      <td><strong>LiD3-Flex</strong></td>
      <td>🟦 <code>7.37 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>4.83 ns / 20 ns</code>; clustering queued</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
    <tr>
      <td><strong>LiND-Hybrid</strong></td>
      <td>🟦 <code>7.33 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>0.78 ns / 20 ns</code>; backfill active</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
    <tr>
      <td><strong>LiLC-1</strong></td>
      <td>🟦 <code>11.28 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>7.58 ns / 20 ns</code>; clustering queued</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
    <tr>
      <td><strong>LiDS-1</strong></td>
      <td>🟦 <code>17.29 ns / 20 ns</code>; clustering queued</td>
    ...
    <tr>
      ...
      <td>🟦 <code>11.35 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>1.08 ns / 20 ns</code>; backfill active</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
    <tr>
      ...
      <td>🟦 <code>11.06 ns / 20 ns</code>; clustering queued</td>
      <td>🟦 <code>7.10 ns / 20 ns</code>; clustering queued</td>
      <td>🟪 planned after paired representatives</td>
      <td>🟪 planned after umbrella sampling</td>
    </tr>
  </tbody>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="center"><strong><code>~7-15 days</code></strong><br><sub>updated after NaCl backfill launch on Worker A</sub></td>
    ...
    <tr>
      ...
      <td align="center"><strong><code>~15-33+ days</code></strong><br><sub>one LiCl representative complete; NaCl is now 8/8 active</sub></td>
      <td>Complete paired PMFs, then compute ΔΔG = ΔG(Na+) - ΔG(Li+) and rank candidates.</td>
    </tr>
  </tbody>
</table>

> Time estimates are conservative and still dominated by NaCl production throughput. The NaCl worker is now progressing under the quota-matched checkpoint-resume configuration; estimates will tighten again after the first queued NaCl jobs enter the active pool.
...
|---:|---|---|---|---|
| 1 | **LiD3-Core** | `GPGDPGPGDPGPGDP` | Linker-free GPGDP trimer benchmark | LiCl `11.15/20 ns`; NaCl `7.25/20 ns` |
| 2 | **LiD3-Flex** | `GPGDPGSGPGDPGSGPGDP` | Flexible GSG-spaced GPGDP trimer | LiCl `7.37/20 ns`; NaCl `4.83/20 ns` |
| 3 | **LiND-Hybrid** | `GPGNPGSGPGDPGSGPGNP` | Mixed GPGNP/GPGDP donor environment | LiCl `7.33/20 ns`; NaCl `0.78/20 ns backfill active` |
| 4 | **LiLC-1** | `GPGDPGSGNPGSGDP` | Lower-charge selectivity-control design | LiCl `11.28/20 ns`; NaCl `7.58/20 ns` |
| 5 | **LiDS-1** | `DGDGPGDPGDG` | Asp/Gly Li+/Na+ geometry probe | LiCl `17.29/20 ns`; NaCl `11.48/20 ns` |
| 6 | **LiDA-1** | `DADGPGDPDAG` | Ala-supported Asp pocket probe | LiCl representative ready, top cluster `17.64%`; NaCl `17.15/20 ns` |
| 7 | **LiN3-Core** | `GPGNPGPGNPGPGNP` | GPGNP trimer benchmark | LiCl `11.35/20 ns`; NaCl `1.08/20 ns backfill active` |
| 8 | **LiA3-Ref** | `GPGAPGPGAPGPGAP` | Low-donor GPGAP reference | LiCl `11.06/20 ns`; NaCl `7.10/20 ns` |
