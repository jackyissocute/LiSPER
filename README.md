...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="center"><strong><code>~1-4 days first paired QC; ~2-5 days broader table</code></strong><br><sub>LiDA-1 and LiDS-1 remain paired QC/repair-review limited. Active V2 windows moved into production on LiD3-Flex, LiD3-Core, LiLC-1, and LiA3-Ref conditions.</sub></td>
...
> Time estimates are based on the current GCP `25` real `mdrun` jobs using `25` OpenMP threads and `86%` data-disk use. All 20 ns production and clustering gates are closed. The first likely paired Delta Delta G table is still contested between LiDA-1 repair review and LiDS-1 paired V2 QC review; no Delta G is promoted before QC passes.
...
|---:|---|---|---|---|
| 1 | **LiD3-Core** | `GPGDPGPGDPGPGDP` | Linker-free GPGDP trimer benchmark | Refined LiCl V2 `12/27`, production `012`, equilibration `013`; NaCl V2 `12/27`, production `012`, equilibration `013` |
| 2 | **LiD3-Flex** | `GPGDPGSGPGDPGSGPGDP` | Flexible GSG-spaced GPGDP trimer | Refined LiCl `21/27`, production `016`, equilibration `017-019`; NaCl `22/27`, production `022-025`, equilibration `026`; PMF QC pending final refined run |
...
| 4 | **LiLC-1** | `GPGDPGSGNPGSGDP` | Lower-charge selectivity-control design | LiCl V2 `3/27`, production `003`; NaCl `V2 12/27`, production `012`, equilibration `013` |
