...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="center"><strong><code>~1-2 days</code></strong><br><sub>one NaCl production tail remains at ~18.14/20 ns</sub></td>
...
> Time estimates are based on the current GCP `28` real `mdrun` jobs using `29` OpenMP threads plus an active LiDS-1 LiCl WHAM/QC driver, and the observed remaining `LiND-Hybrid` NaCl production tail at `18.14/20 ns`. The first likely paired Delta Delta G table is now contested between LiDA-1 repair review and LiDS-1 paired V2 WHAM/QC; no Delta G is promoted before QC passes.
...
- GCP is active with `28` real `mdrun` processes using `29` OpenMP threads plus an active LiDS-1 LiCl WHAM/QC driver. Recent AutoDL checkpoint/log evidence was archived on the GCP data disk before shutdown planning, so the handoff does not depend on live AutoDL state.
...
| 3 | **LiND-Hybrid** | `GPGNPGSGPGDPGSGPGNP` | Mixed GPGNP/GPGDP donor environment | LiCl representative ready; NaCl GCP backfill active at `18.14/20 ns` |
